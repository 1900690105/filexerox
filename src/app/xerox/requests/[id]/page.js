...
export default function RequestDetailsPage() {
  ...
  useEffect(() => {
    const fetchRequests = async () => {
      try {
        ...
        // Create query to get documents where xeroxCenterCode equals "2025"
        const q = query(
          collection(db, "userfiles"),
          where("xeroxCenterCode", "==", "2025")
        );
  ...
  const getFileIcon = (filename) => {
    ...
    switch (extension) {
      ...
      case "jpg":
      case "jpeg":
        return <ImageDown className="w-5 h-5 text-green-500" />;
      default:
                    ...
                    <p className="text-xs text-gray-400">
                      ...
                      <a
                        href={`https://docs.google.com/viewer?url=${encodeURIComponent(
                          window.location.origin + selectedFile.name
                        )}&embedded=true`}
                        target="_blank"
                        className="text-blue-500 hover:underline"
                      >
              ...
              <div className="grid grid-cols-2 md:grid-cols-3 gap-4 text-sm">
                <div>
                  ...
                  <span className="ml-2 font-medium capitalize">
                    {selectedFile.settings?.color?.replace("_", " ") || "N/A"}
                  </span>
                </div>
                <div>
                  <span className="text-gray-500">Sides:</span>
                  <span className="ml-2 font-medium capitalize">
                    {selectedFile.settings?.sides?.replace("_", " ") || "N/A"}
                  </span>
                </div>
                <div>
                  <span className="text-gray-500">Paper Size:</span>
                  <span className="ml-2 font-medium">
                    {selectedFile.settings?.paperSize || "N/A"}
                  </span>
                </div>
                <div>
                  <span className="text-gray-500">Pages per Sheet:</span>
                  <span className="ml-2 font-medium">
                    {selectedFile.settings?.pagesPerSheet || "N/A"}
                  </span>
                </div>
                <div>
                  <span className="text-gray-500">Copies:</span>
                  <span className="ml-2 font-medium">
                    {selectedFile.settings?.numberOfCopies || "N/A"}
                  </span>
                </div>
                <div>
                  <span className="text-gray-500">Pages:</span>
                  <span className="ml-2 font-medium">
                    {selectedFile.settings?.pages || "N/A"}
                  </span>
                </div>
              </div>
            </div>
    ...
    return (
      <div className="min-h-screen bg-gray-50 flex items-center justify-center">
        <div className="text-center">
          ...
          <p className="text-gray-600">Loading print requests...</p>
        </div>
      </div>
    );
  }
      ...
      <div className="max-w-6xl mx-auto">
        ...
        <div className="mb-8">
          ...
          <p className="text-gray-600">
            Manage and preview your print requests (Xerox Center: 2025)
          </p>
        </div>
        ...
        {request.length === 0 && (
          <div className="text-center py-12">
            ...
            <p className="text-gray-500">
              No requests found for xerox center code `&#34;2025`&#34;.
            </p>
          </div>
        )}
              ...
              {req?.printingOptions && (
                <div className="p-6">
                  <div className="space-y-3">
                    {req?.printingOptions?.map((file, index) => (
                      <div
                        key={index}
                        ...
                        <div className="flex items-center space-x-3">
                          ...
                          <div>
                            ...
                            <div className="flex items-center space-x-4 text-xs text-gray-500 mt-1">
                              ...
                              <span>{file.settings?.paperSize || "N/A"}</span>
                              <span>
                                {file.settings?.numberOfCopies || "N/A"} copies
                              </span>
                              <span>
                                Pages: {file.settings?.pages || "N/A"}
                              </span>
                            </div>
                          </div>
                        </div>
